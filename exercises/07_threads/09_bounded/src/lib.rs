...
// The solution uses thiserror crate
...
impl TicketStoreClient {
    pub fn insert(&self, draft: TicketDraft) -> Result<TicketId, OverloadedError> {
        ...
        // because now the return is a Result<T,Error> type, so we use Ok() to extract the T (TicketId) when the result is Ok
...
enum Command {
    Insert {
        draft: TicketDraft,
        // change from Sender to SyncSender
