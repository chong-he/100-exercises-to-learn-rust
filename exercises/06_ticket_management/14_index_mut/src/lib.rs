...
// note: impl Index is already implemented, below there, so we can impl IndexMut right away
impl IndexMut<TicketId> for TicketStore {
    fn index_mut(&mut self, index: TicketId) -> &mut Self::Output {
        // refer to get method, because we want to return &mut, so we use .iter_mut() instead of .iter()
