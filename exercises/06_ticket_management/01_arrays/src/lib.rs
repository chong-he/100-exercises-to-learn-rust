...
impl WeekTemperatures {
    pub fn new() -> Self {
        // from the test, we see: assert_eq!(week_temperatures.get_temperature(Weekday::Monday), None);
        // so before any temperature is set, the temperature is None
        // therefore, we put the temperature as Option <i32>, not just i32
        WeekTemperatures {
            temperature: [None; 7],
        }
    }
    ...
    pub fn get_temperature(&self, day: Weekday) -> Option<i32> {
        self.temperature[weekday_to_index(day)]
    }

    pub fn set_temperature(&mut self, day: Weekday, temperature: i32) {
        let index = weekday_to_index(day);
        // temperature is an array with 7 elements, corresponding to each day of the week
        // so self.temperature is an array, with elements either Option<i32> or None
        // when we set the temperature, we want to set it for a particular day (the index)
