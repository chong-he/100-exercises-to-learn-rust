...
fn easy_ticket(title: String, description: String, status: Status) -> Ticket {
    match Ticket::new(title.clone(), description, status.clone()) {
        Ok(ticket) => ticket,
        Err(e) => match e {
            // When Ticket errors, it is now of type TicketNewError, not String anymore (as in previous section)
            // and TicketNewError only contains 2 variants, so we can use match to match for each case
...
impl Ticket {
    pub fn new(
        ...
        if title.is_empty() {
            // In previous section (chap5sec07), the error is a String, so we did "return Err (String)"
            // Now the error is a TicketNewError type, so we return Err(TicketNewError)
